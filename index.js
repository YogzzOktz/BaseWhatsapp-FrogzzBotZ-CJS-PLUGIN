...
async function requestPairing(FrogzzBotZ, state) {
  ...
  try {
    const configured = process.env.BOT_NUMBER || await ask('Nomor WhatsApp untuk pairing: ')
  ...
  try {
    ...
    FrogzzBotZ.ev.on('connection.update', async update => {
      ...
      if (!scheduleReconnect()) {
        process.stdout.write(`${BotInfo.name} gagal reconnect ${MAX_RECONNECT_ATTEMPTS} kali. Bot dimatikan. Hapus folder session jika ingin pairing ulang.\n`)
      }
    })
